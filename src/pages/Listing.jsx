...
function Listing() {
  const regularExpression = /\B(?=(\d{3})+(?!\d))/g
    ...
    <main>
      ...
      <div className='listingDetails'>
        <p className='listingName'>
          {listing.name} - ${listing.offer ? listing.discountedPrice.toString().replace(regularExpression, ',') : listing.regularPrice.toString().replace(regularExpression, ',')}
        </p>
        ...
        <ul className='listingDetailsList'>
          <li>{listing.bedroom > 1 ? `${listing.bedrooms} Bedrooms` : '1 Bedroom'}</li>
        ...
        <div className='leafletContainer'>
          <MapContainer style={{ width: '100%', height: '100%' }} center={[listing.geolocation.lat, listing.geolocation.lng]} zoom={13} scrollWheelZoom={false}>
